...
- **🔌 충전기(AC) 연결 시**: 터보 부스트 ON (65% 클럭 제한) + 균형 모드 + EPP `balance_performance`
  - P코어 ~2.9GHz / 8E코어 ~2.34GHz로 구동되어 총 24.5 GHz·core의 데스크톱급 멀티코어 성능 발휘
- **🔋 배터리(DC) 전환 시**: 터보 부스트 OFF (기본 클럭 100%) + 균형 모드 + EPP `power`
  - P코어 ~2.00GHz / 8E코어 ~1.00GHz로 구동되며, 하드웨어 에너지 정책(EPP)을 최저 전력 단계(`power`)로 강제하여 P코어의 과도한 주파수 램프업을 억제하고 C10 딥슬립 진입 극대화 (실측 평균 10.16W, 바닥 7.13W 달성)
...
| **터보 OFF (100%) + EPP power 🏆** | **2P + 8E (10코어)** | **100% 베이스** | **10.16 W** | **7.13 W** | **11.42 W** | **2.01 GHz** | **1.01 GHz** | **44.0 °C** | **최종 선정**: EPP power로 피크 스파이크 원천 억제 (-4.9W 절감) |
...
2. **하드웨어 에너지 정책(EPP)의 `power` 강제 효과**:
   - `powerprofilesctl set balanced` 실행 시 커널 기본값으로 복귀되는 EPP(`balance_power`)를 하드웨어 최저 전력 선호도인 `power`로 명시적 재설정.
   - P코어가 사소한 부하에 1.9~2.0GHz 풀클럭으로 치솟는 빈도를 줄이고 8개 E코어 및 C10 딥슬립을 최우선 활용하도록 유도하여, 피크 소비전력을 16.9W에서 11.4W로 억제하고 평균 전력을 10.1W까지 낮춤.

3. **다이 상의 Dark Silicon 발열 차단**:
