...
echo " (driver_power_patch: i915 안정화, Microcode, OpenCL, ASPM, PowerTOP)"
echo "========================================================="
...
echo " 2. 검증된 i915 그래픽 드라이버 안정화 (Early KMS):"
echo "    - xe의 실험적 버그(GPU TLB 타임아웃, 절전 복귀 락 실패) 원천 차단"
echo "    - Early KMS dracut 탑재: force_drivers+=\" i915 \""
echo "    - 부팅 극초기 i915 선행 로드로 ACPI/부팅 레이스 컨디션(부팅 정체) 완전 해결"
echo "    - SOF 스피커 오디오 및 OLED 다중 주사율/sdr-native 클램핑과 완벽 호환"
echo " 3. GPU 하드웨어 연산 가속 (OpenCL Compute Runtime):"
echo "    - intel-opencl-icd & clinfo 탑재 (i915 기반 Intel Arc GPU 연산 가속 활성화)"
...
echo ""
echo " 🚀 필수 안내:"
echo " 1. 커널 파라미터 및 부팅 램디스크 설정을 시스템에 완전히 적용하려면"
echo "    재부팅이 필요합니다:"
echo "    sudo reboot"
echo ""
echo " 2. 재부팅 후 적용 상태 확인 명령어:"
echo "    - 그래픽 드라이버 확인: lsmod | grep i915"
