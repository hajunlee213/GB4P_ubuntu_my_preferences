...
### 1. CPU 하이브리드 아키텍처 토폴로지 최적화 (E-코어 8개 전담 + CPU 0 식물인간 격리)
* **P-core (CPU 1~7) 완전 차단**:
  * P코어 물리 코어 및 하이퍼스레딩(CPU 1, 2, 3, 4, 5, 6, 7)을 하드웨어적으로 오프라인
* **CPU 0 식물인간 격리 (x86 BSP 제약 극복 & C10 딥슬립 유지)**:
...
* **E-core 8개 전체 가동 (Cluster 0 & Cluster 1, CPU 8~15 ON)**:
  * 브라우저 및 일상적인 모든 작업을 E-코어 8개가 전담 처리
  * P-코어의 거대한 OOOE(비순차적 실행) 구조와 국소 핫스팟이 원천 배제되어 발열이 다이 전체에 고르게 분산되고 42°C대 무소음(팬 0 RPM 정지) 유지
* **LP-E Core (CPU 16, 17) 차단 (Compute 타일 단일화 & 크로스 타일 오버헤드 차단)**:
  ...
  * SoC 타일과 Compute 타일 간 패브릭 인터커넥트 통신 비용 및 캐시 스눕 전력 낭비를 완전히 제거하고, 모든 활성 코어(8E)를 18MB L3를 공유하는 **Compute 타일 내부로 단일화**
...
| **터보 OFF (100%) + EPP power** | 2P + 8E (10코어) | 100% 베이스 | **10.16 W** | 7.13 W | 11.42 W | 2.01 GHz | 1.01 GHz | 44.0 °C | EPP power로 피크 스파이크 원천 억제 (-4.9W 절감) |
| **8E 전담 (60% 측정 / 70% 실사용) + CPU 0 격리** | **8E (CPU 8~15)** | **60% 제한 (~2.0GHz)** | **13.44 W** (CPU 5.4W) | **6.53 W** (CPU 2.9W) | **15.20 W** | **N/A (C10 격리)** | **2.00 GHz** | **42.0 °C** | **최종 선정**: P코어 원천 배제로 42°C 무소음 달성, 실사용은 70%(~2.5GHz, 20 GHz·core)로 상향 |
...
1. **E-코어 8개 전담 + CPU 0 식물인간 격리 아키텍처의 혁신**:
   ...
   - CPU 0을 C10 딥슬립으로 격리하고 CPU 1~7을 오프라인한 뒤 8개 E-코어를 가동하면, 웹서핑+음악 재생 부하에서도 CPU 온도가 42.0°C에 고정되어 쿨링팬이 0 RPM으로 완벽히 정지합니다.
   - 초기 실측 당시 60% 제한(~2.0GHz, 16.0 GHz·core)으로 검증을 완료하였으며, 현재 실사용 설정은 멀티태스킹 반응성을 더 높이기 위해 전성비 최적 구간인 **70% 제한(~2.5GHz, 8코어 총 20.0 GHz·core)**으로 상향 적용되어 있습니다. 이는 기존 2P(2.0GHz)+8E(1.0GHz)=12.0 GHz·core 대비 66% 이상 여유로운 연산력을 제공하면서도 열역학적 쾌적성과 배터리 지속 시간을 극대화합니다.

2. **"저클럭 다코어(8E @ 1.0GHz)" vs "고클럭 소수코어(4E @ 1.5GHz)"**:
   - 현대 웹 브라우저(크롬/파이어폭스)는 탭 하나만 열어도 JS V8 엔진, GC, 컴포지터, 네트워크 등 수십 개의 백그라운드 스레드를 생성합니다.
   - 4E 환경에서는 4개 코어에 부하가 집중되어 스케줄러가 P코어를 자주 깨워 피크 전력이 18.9W까지 튑니다.
   - 반면 8E 환경에서는 8개 코어가 일감을 넉넉히 분산 처리하여 P코어 개입을 최소화하고, 피크(16.9W)와 바닥(8.4W)을 모두 낮춥니다.

3. **코어 오프라인 조합의 하드웨어적 설계 근거**:
   - **P-core 완전 차단 (CPU 1~7 OFF)**: 발열과 전력의 주범인 P-코어 다이를 원천 비활성화하여 쿨링팬 소음 완전 제거.
   - **CPU 0 식물인간 격리**: x86 커널 특성상 오프라인 불가한 CPU 0에 프로세스 배정을 원천 차단하여 순수 C10 딥슬립 상태 유지.
   - **LP-E 차단 (CPU 16, 17 OFF)**: SoC 타일과 Compute 타일 간 패브릭 인터커넥트 병목 및 슬립 낭비 원천 제거.

4. **AC/DC 단일 토폴로지 무지연 전환**:
   - 부팅 시 시스템을 **[8E 전담 + CPU 0 격리]**로 단일화하여, 충전기 탈착 시 CPU Hotplug(코어 켜고 끄기) 오버헤드 없이 `intel_pstate` 레지스터 전환만으로 0.001초 만에 매끄럽게 AC(터보 80% / ~2.8~3.0GHz) $\leftrightarrow$ DC(터보 70% / ~2.5GHz)를 전환합니다.
