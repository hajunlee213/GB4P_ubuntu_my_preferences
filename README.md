...
* **DC 모드 (배터리 사용 시)**:
  * **터보 부스트**: ON (`no_turbo = 0`)
  * **클럭 상한선**: 40% (`max_perf_pct = 40`) - P코어 ~2.00GHz / E코어 ~1.50GHz (실측 최적 스윗스팟)
  * **동작 전략**: E코어 1.0GHz 족쇄를 풀어 작업을 빠르게 끝내고 C-state 깊은 절전으로 복귀하는 **Race to Sleep 극대화** 및 P코어 저전압 고효율 구간 유지로 배터리 전력 12W대 진입
