...
* **DC 모드 (배터리 사용 시)**:
  * **터보 부스트**: ON (`no_turbo = 0`)
  * **클럭 상한선**: 60% (`max_perf_pct = 60`) - E-코어 ~2.0GHz * 8개 (총 16.0 GHz·core의 넉넉한 연산력)
  * **에너지 정책**: EPP `power` (하드웨어 최저 전력 선호도 강제)
  * **동작 전략**: P-코어 발열을 원천 배제하고 8개 E-코어로 부하 분산 (실측 42.0°C 무소음 극저발열, 팬 0 RPM 완전 정지 및 배터리 절감)
...
4. **AC/DC 단일 토폴로지 무지연 전환**:
   - 부팅 시 시스템을 **[8E 전담 + CPU 0 격리]**로 단일화하여, 충전기 탈착 시 CPU Hotplug(코어 켜고 끄기) 오버헤드 없이 `intel_pstate` 레지스터 전환만으로 0.001초 만에 매끄럽게 AC(터보 80% / ~3.0GHz) $\leftrightarrow$ DC(터보 60% / ~2.0GHz)를 전환합니다.
